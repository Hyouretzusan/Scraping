...
class textfiles_management:
    ...
    def bdd_creartabla():
        ...
        cur.execute('CREATE TABLE Directorio (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL UNIQUE, link TEXT UNIQUE, nivel INTEGER)')
        cur.execute('DROP TABLE IF EXISTS Subdirectorio')
        cur.execute('CREATE TABLE Subdirectorio (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL UNIQUE, link TEXT UNIQUE, nivel INTEGER, directorio_id INTEGER, UNIQUE(id, directorio_id))')
        cur.execute('DROP TABLE IF EXISTS Textos')
        cur.execute('CREATE TABLE Textos (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL UNIQUE, link TEXT UNIQUE, nivel_id INTEGER, padre_id INTEGER, UNIQUE(id, nivel_id))')
